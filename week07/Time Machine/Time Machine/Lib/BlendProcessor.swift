...
class BlendProcessor {
    ...
    func generateBlend(backImage: UIImage, foreImage: UIImage) {
        guard
            ...
            let foregroundImage = foreImage.cgImage
        else {
        ...
        do {
            ...
            let foreground = CIImage(cgImage: foregroundImage).oriented(
                forExifOrientation: Int32(foreImage.imageOrientation.rawValue))
            let maskImage = CIImage(cvPixelBuffer: mask.pixelBuffer)
            let background = CIImage(cgImage: backgroundImage).oriented(
                forExifOrientation: Int32(backImage.imageOrientation.rawValue))

            guard
                let output = blendImages(
    ...
    func blendImages(
        ...
        let backgroundScaleX =
            (foreground.extent.width / background.extent.width)
        let backgroundScaleY =
            (foreground.extent.height / background.extent.height)
        let backgroundScaled = background.transformed(
            by: __CGAffineTransformMake(
